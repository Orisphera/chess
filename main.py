...
def draw_piece(btn, piece):
    btn.configure(text=('' if piece is None else piece.picture()))
...
class Board:
    def __init__(self):
        ...
        self.rev_moves_p1 = 1
        self.tk_other = []
        self.tk_promote_btns = []
        self.tk_field = []
        self.tk_promote_bar = self.tk_board = self.tk_help = self.tk_message = self.tk_window = None

    def disable_btn(self, row, col):
        btn = self.tk_field[row][col]
        bg = "#FFFFFF" if (row + col) % 2 else "#707070"
        btn.configure(state=tk.DISABLED, command=lambda: None, bg=bg)
        try:
            btn.configure(disabledbackground=bg)
        except TclError:
            pass

    def enable_btn(self, row, col, command):
        btn = self.tk_field[row][col]
        btn.configure(state=tk.ACTIVE, command=command, bg=btn.configure('activebackground')[-1])
    ...
    def init_gui(self):
        ...
        self.tk_window.title('Шахматы')
        self.tk_window.geometry("700x660")
        self.tk_help = tk.Label(text='Поля, доступные для хода, выделены цветом\n'
                                     'Для рокировки сходите королём')
        self.tk_help.pack()
        self.tk_message = tk.Label()
        self.tk_message.pack()
        self.tk_promote_bar = tk.Frame()
        self.tk_promote_bar.pack()
        self.tk_board = tk.Frame()
        bigger = Font(size=20)
        for row in range(8):
            field_row = []
            self.tk_field.append(field_row)
            grid_row = 8 - row
            for col in range(8):
                bg = ("#FFD0D0" if (row + col) % 2 else "#A07878")
                btn = tk.Button(self.tk_board, activebackground=bg, disabledforeground='#000000',
                                font=bigger, width=3)
                btn.grid(row=grid_row, column=col + 1, sticky=tk.NSEW)
                field_row.append(btn)
            row1 = row + 1
            lbl1 = tk.Label(self.tk_board, text=str(row1))
            lbl2 = tk.Label(self.tk_board, text=str(row1))
            lbl3 = tk.Label(self.tk_board, text=chr(ord('a') + row))
            lbl4 = tk.Label(self.tk_board, text=chr(ord('a') + row))
            lbl1.grid(row=grid_row, column=0)
            lbl2.grid(row=grid_row, column=9)
            lbl3.grid(row=0, column=row1)
            lbl4.grid(row=9, column=row1)
        self.tk_board.pack()
        self.reset_gui()
